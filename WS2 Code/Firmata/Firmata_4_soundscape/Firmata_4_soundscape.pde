...
String[] fileNames = {"birds", "frogs", "nature", "rain", "woodpecker"}; 
...
float [] previousChannelRate = new float[fileNames.length];
...
void setup() {
  ...
  for (int i = 0; i < fileNames.length; i++) {
    ...
    previousChannelRate[i] = 1;
    sounds[i].amp(previousChannelVolume[i]);
    sounds[i].rate(previousChannelRate[i]);
  }
...
void draw() {
  ...
  sounds[channel].rate(speed);

  //If channel is changed, save the current volume and rate on that channel
  if (button_a.change || button_b.change) {
    previousChannelVolume[channel] = volume;
    previousChannelRate[channel] = speed;
  }
  //Change channels
  if (button_a.change) {
    channel--;
